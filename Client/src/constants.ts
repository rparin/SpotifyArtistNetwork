const APP_NAME = "Spotify Artist Network";

const MY_NETWORK_BTN_TEXT = "View My Network";

const SEARCH_PLACEHOLDER =
  "Enter the name of an artist or provide a Spotify link";
const SHORT_SEARCH_PLACEHOLDER = "Search Artist or enter Spotify link";

const FOOTER_SEARCH_TEXT = "How search works";
const FOOTER_LOGIN_TEXT = "Why login?";
const CR = "©️ 2023. Developed by Ralph Parin.";

export {
  APP_NAME,
  MY_NETWORK_BTN_TEXT,
  SEARCH_PLACEHOLDER,
  SHORT_SEARCH_PLACEHOLDER,
  FOOTER_SEARCH_TEXT,
  FOOTER_LOGIN_TEXT,
  CR,
};
